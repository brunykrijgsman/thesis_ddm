...
import os
from integrative_ddm_sim import prior, likelihood
...
def generate_factorial_ddm_data(ntrials=100, nparts=100, seed=2025):
    """
    Generate data across 2x2x3 factorial design using integrative DDM:
    - SNR (low/high): controls sigma (P300 noise)
    - Coupling (low/high): controls gamma (DDM-P300 coupling)
    - Error distribution: laplace, gaussian, uniform
    -------------------------------------------------------------------------------------
    ...
    for snr in snr_levels:
        for coupling in coupling_levels:
            for dist in error_distributions:
                ...
                gamma_all = np.random.uniform(gamma_min, gamma_max, size=nparts) * signs
                ...
                for p in range(nparts):
                    ...
                    sim_data = likelihood(
                        params['alpha'], params['tau'], params['beta'],
                        params['mu_delta'], params['eta_delta'], 
                    ...
                    if dist == 'gaussian':
                        z_noisy = np.random.normal(params['gamma'] * delta, params['sigma'], ntrials)
                    elif dist == 'laplace':
                        b_laplace = params['sigma'] / np.sqrt(2)
                        z_noisy = np.random.laplace(params['gamma'] * delta, b_laplace, ntrials)
                    elif dist == 'uniform':
                        a_uniform = (params['gamma'] * delta) - np.sqrt(3) * params['sigma']
                        b_uniform = (params['gamma'] * delta) + np.sqrt(3) * params['sigma']
                ...
                filename = os.path.join(data_dir, f"integrative_ddm_data_{condition_key}.mat")
...
if __name__ == "__main__":
    ...
    all_data, condition_labels = generate_factorial_ddm_data()
