...
if not os.path.exists(data_filename) or True:
    ...
    for p in range(nparts):
        ...
        tempout, _ = simul_directed_ddm(ntrials=ntrials, alpha=alpha[p], tau=tau[p], beta=beta[p],
        ...
        # Update delta_i (drift rate) influenced by P300
        delta_i = np.random.normal(lambda_param[p] * z_i + b[p], eta[p], size=ntrials)
        
        # Initialize tracker
        start = indextrack
        end = indextrack + ntrials

        # Update y_i (response time)
